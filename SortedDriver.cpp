...
{
	double prev, difference1,difference2,next,result,isolated;
	double  min= 0;
	...
	for (int i = 0; i < number.size(); i++)
	{
		...
		else
		{
			next = number.at(i + 1);
			prev = number.at(i - 1);
			difference1 = current - prev;
			difference2 = next - current;
...
double count = 0;
for (std::list<string>::iterator itA = A.begin(),  itB = B.begin(); itA != A.end(); )
{
	if (*itA < *itB)
	{
		itA++;
